...
namespace lrl2
{
  ...
  LRL2Solver::LRL2Solver (petuum::TableGroup < float >*table_group,
			  ...
			  const LRL2SolverConfig & config):_config (config),
    _thread_counter (0)
  {
  ...
  void LRL2Solver::Solve ()
  {
    ...
    CHECK_EQ (0,
	      _table_group->RegisterThread ()) << "Failed to register thread";
    ...
    int32_t num_data_per_thread =
      static_cast < int32_t > ((_num_data + num_threads - 1) / num_threads);
	...
	if (_solver_thread_data->thread_id == _config.num_threads - 1)
	  ...
	  {
	    _solver_thread_data->data_idx_end =
	      _solver_thread_data->data_idx_begin + num_data_per_thread;
	  }
      }

    int idx_begin = _solver_thread_data->data_idx_begin;
    int idx_end = _solver_thread_data->data_idx_end;
    LOG (INFO) << "Thread " << _solver_thread_data->
      thread_id << " is solving " << idx_begin << " ~ " << idx_end;
	...
	std::cout << "true_pos:" << true_pos << "\tfalse_pos:" << false_pos
	  << "\ttrue_neg:" << true_neg << "\tfalse_neg:" << false_neg << std::
	  endl;

	float precision = ((float) (true_pos)) / (true_pos + false_pos);
	float recall = ((float) (true_pos)) / (true_pos + false_neg);
	std::cout << "precision:" << precision << "recall:" << recall << std::
	  endl;
...
//modify dai wei's code, change vector to valarray
			     ...
			     std::vector < int32_t > &Y)
  {
    ...
    free (line);
    CHECK_EQ (0,
	      fclose (data_stream)) << "Failed to close file " << data_file;
  ...
  int32_t LRL2Solver::SolveBatch (int32_t local_idx)
  {
    ...
    int size =
      _solver_thread_data->data_idx_end - _solver_thread_data->data_idx_begin;
  ...
  int32_t LRL2Solver::SolveBatchPartialWeights (int32_t local_idx)
  {
	...
	for (int j = 0; j < weight_dim; j++)
	  grad_active[j] +=
	    (sig -
	     1.f) * (float) (_Y[data_idx]) * _X[data_idx][loc[_aug_dim - j -
							      1]];
	local_idx = (++local_idx) % size;
      }
    for (int j = 0; j < weight_dim; j++)
