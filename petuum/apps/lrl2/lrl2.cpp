...
namespace lrl2 {
	...
	LOG(INFO) << "done reading dataset from " << _config.
	    train_data_file;
	srand(time(NULL));
    }
	...
	CHECK_EQ(0,
		 _table_group->
		 RegisterThread()) << "Failed to register thread";
	...
	if (_config.client_rank == 0
		...
		float obj = 0.5 * _lambda * dot_product(w, w) + loss;
	    ...
	    std::
		cout << "true_pos:" << true_pos << "\tfalse_pos:" <<
...
//modify dai wei's code, change vector to valarray
    ...
    float LRL2Solver::ComputeTestError(std::vector < std::valarray <
				       float > >&X_test,
