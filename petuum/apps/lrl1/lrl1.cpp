...
namespace lrl1 {
    ...
    LRL1Solver::LRL1Solver(petuum::TableGroup < float >*table_group,
			   ...
			   const LRL1SolverConfig &
			   config):_config(config), _thread_counter(0) {
	...
	LOG(INFO) << "done reading dataset from " <<
	    _config.train_data_file;
	srand(time(NULL));
    }
	...
	int32_t num_data_per_thread =
	    static_cast < int32_t >
	    ((_num_data + num_threads - 1) / num_threads);
	...
	if (!_solver_thread_data.get()) {
	    ...
	    _solver_thread_data->data_idx_begin =
		_solver_thread_data->thread_id * num_data_per_thread;
	    if (_solver_thread_data->thread_id == _config.num_threads - 1) {
		//TODO:what's this?
		_solver_thread_data->data_idx_end =
		    _config.data_global_idx_begin + _num_data;
	    } else {
		_solver_thread_data->data_idx_end =
		    _solver_thread_data->data_idx_begin +
		    num_data_per_thread;
	...
	LOG(INFO) << "Thread " << _solver_thread_data->thread_id <<
	    " is solving " << idx_begin << " ~ " << idx_end;
	...
	for (int iter = 0; iter < max_iter && !converged; iter++) {
		...
		if (_solver_thread_data->data_idx_begin == 0) {
		    ...
		    petuum::DenseRow < float >&w_row =
			_w_table->GetRowUnsafe(0);
		    for (int dim = 0; dim < _aug_dim; ++dim) {
			_w_snapshot_table->Put(evaluation_count, dim,
					       w_row[dim]);
		    }
		}
		++evaluation_count;
	...
	if (_config.client_rank == 0
	    ...
	    //test
	    std::vector < std::valarray < float >>X_test;
	    std::vector < int >Y_test;
	    ReadData(_config.test_data_file, X_test, Y_test);
	    int true_pos = 0;
	    int true_neg = 0;
	    int false_pos = 0;
	    int false_neg = 0;
	    for (int i = 0; i < X_test.size(); i++) {
		float sig = sigmoid(X_test[i], w, 1.f);
		if (sig > 0.5) {
		    if (Y_test[i] == 1)
			true_pos++;
		    else
	    ...
	    std::cout << "true_pos:" << true_pos << "\tfalse_pos:" <<
		false_pos << "\ttrue_neg:" << true_neg << "\tfalse_neg:" <<
		false_neg << std::endl;
...
//modify dai wei's code, change vector to valarray
	...
	while (getline(&line, &num_bytes, data_stream) != -1) {
	    ...
	    CHECK_LE(label,
		     1) << "data " << _num_data << " has label " << label
		<< " out of range.";
	    CHECK_LE(-1,
		     label) << "data " << _num_data << " has label " <<
		label << " out of range.";
	...
	free(line);
	CHECK_EQ(0,
		 fclose(data_stream)) << "Failed to close file " <<
	    data_file;
    ...
    inline float dot_product(std::valarray < float >&record,
			     std::valarray < float >&weights) {
	return (record * weights).sum();
    };

    inline float inv_sigmoid(std::valarray < float >&record,
			     std::valarray < float >&weights,
			     float label) {
	return 1.f + exp(-dot_product(record, weights) * label);
    };

    inline float sigmoid(std::valarray < float >&record,
			 std::valarray < float >&weights, float label) {
...
#define ZERO_GUARD 0.00001f
    ...
    int32_t LRL1Solver::SolveBatch(int32_t local_idx) {
	petuum::DenseRow < float >&w_row = _w_table->GetRowUnsafe(0);
	std::valarray < float >w(_aug_dim);
	for (int i = 0; i < _aug_dim; i++) {
	    w[i] = w_row[i];
	...
	for (int i = 0; i < _config.batch_size; i++) {
	    int32_t data_idx =
		local_idx + _solver_thread_data->data_idx_begin;
	    float sig = sigmoid(_X[data_idx], w, _Y[data_idx]);
	    grad += (sig - 1.f) * (float) (_Y[data_idx]) * _X[data_idx];
    ...
    int32_t LRL1Solver::SolveBatchPartialWeights(int32_t local_idx) {
	...
	for (int i = 0; i < _config.batch_size; i++) {
		...
		grad_active[j] +=
		    (sig -
		     1.f) * (float) (_Y[data_idx]) *
		    _X[data_idx][loc[_aug_dim - j - 1]];
	    local_idx = (++local_idx) % size;
	}
	for (int j = 0; j < weight_dim; j++) {
	    float subgrad = 0.f;
	    if (w[loc[_aug_dim - j - 1]] < -ZERO_GUARD)
		subgrad = -1.f;
	    else if (w[loc[_aug_dim - j - 1]] > ZERO_GUARD)
		subgrad = 1.f;
	    else {
		if (grad_active[j] < -_lambda)
		    subgrad = 1.f;
		else if (grad_active[j] > _lambda)
		    subgrad = -1.f;
		else
		    subgrad = 0.f;
	    }
	    grad_active[j] += subgrad * _lambda;
    ...
    float LRL1Solver::ComputeTestError(std::vector < std::valarray <
				       float > >&X_test,
				       std::vector < int32_t > &Y_test,
	...
	return static_cast < float >(error_count) / static_cast <
	    float >(num_tests);
    }
